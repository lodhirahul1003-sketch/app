  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto bg-white border-t border-slate-100 p-4 flex justify-around items-center z-30">
        ...
        <button onClick={() => handleNavigate('/family')} className="flex flex-col items-center gap-1 text-primary">
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="relative w-full max-w-[430px] bg-white rounded-t-[32px] p-8 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { hapticFeedback.light(); setShowAddModal(false); }}><X className="w-6 h-6 text-slate-400" /></button>
              </div>
              ...
              <button onClick={() => { hapticFeedback.success(); setShowAddModal(false); }} className="w-full py-4 bg-primary text-white rounded-2xl font-bold shadow-lg shadow-primary/30 active:scale-[0.98] transition-all">Send Invitation</button>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white rounded-3xl p-8 text-center space-y-6">
              ...
              <div className="flex gap-3">
                <button onClick={() => { hapticFeedback.light(); setShowDeleteModal(false); }} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl">Cancel</button>
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white rounded-3xl p-8 text-center space-y-6">
              ...
              <button onClick={() => { hapticFeedback.light(); setShowSuccessModal(false); }} className="w-full py-4 bg-primary text-white rounded-2xl font-bold shadow-lg shadow-primary/30 active:scale-[0.98] transition-all">Close</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
