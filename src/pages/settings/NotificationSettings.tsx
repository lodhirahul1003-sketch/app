...
const NotificationSettings = () => {
  ...
  const [settings, setSettings] = useState([
    { 
      ...
      items: [
        { id: 'out-for-delivery', icon: Package, label: 'Out for Delivery', desc: 'Notify when package is on its way', active: true },
        { id: 'arrived-at-door', icon: Bell, label: 'Arrived at Door', desc: 'Notify when delivery partner arrives', active: true },
        { id: 'delivery-updates', icon: Info, label: 'Delivery Updates', desc: 'Notify about delays or rescheduling', active: false },
      ]
    },
    { 
      title: 'Security & Access',
      items: [
        { id: 'unauthorized-access', icon: Shield, label: 'Unauthorized Access', desc: 'Alert if someone tries to force open', active: true },
        { id: 'family-entry', icon: User, label: 'Family Member Entry', desc: 'Notify when family members arrive', active: false },
        { id: 'low-battery', icon: Bell, label: 'Low Battery Alert', desc: 'Notify when smart hub battery is low', active: true },
      ]
    }
  ]);
  ...
  const toggleSetting = (sectionIndex: number, itemIndex: number) => {
    ...
    newSettings[sectionIndex].items[itemIndex].active = !newSettings[sectionIndex].items[itemIndex].active;
          ...
          <div key={i} className="space-y-4">
            ...
            <div className="space-y-3">
              {section.items.map((item, j) => (
                <button 
                  key={j} 
                  onClick={() => toggleSetting(i, j)}
                  className="w-full bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 text-left active:scale-[0.98] transition-all"
                >
